...
data$generation_twh = factor(
  data$generation_twh,
  levels = c("Bioenergy","Oil","Unabated coal","Unabated natural gas"),
  labels = c("Bioenergy","Oil","Unabated Coal","Unabated Natural Gas")
)
...
ggplot(data, aes(x = generation_twh, y = co2_g_kwh/1e3, fill=scenario)) +
  ...
  labs(title=expression(paste("IEA Emissions by Electricity Generation Source (", CO[2], ").")),
       fill=NULL,
       subtitle = "Reported by region for IEA policy scenarios (Eurasia/Russia SPS2022 bioenergy removed).",
...
# "Africa","Asia Pacific","Brazil","Central and South America", "China",
# "Eurasia","Europe","European Union","India","Japan","Middle East","Non-OECD",                 
# "North America","Russia","Southeast Asia","United States" 

data$product = factor(
  data$product,
  levels = c("Coal: unabated","Natural gas: unabated","Nuclear","Oil","Renewables"),
  labels = c("Unabated Coal","Unabated Natural Gas","Nuclear","Oil","Renewables")
)
# data$product = factor(
#   data$product,
#   levels = c("Renewables","Oil","Nuclear","Natural gas: unabated","Coal: unabated"),
#   labels = c("Renewables","Oil","Nuclear","Unabated Natural Gas","Unabated Coal")
# )
df_errorbar <-
  data |>
  group_by(SCENARIO, product, REGION) |>
