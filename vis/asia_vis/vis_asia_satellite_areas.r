...
data = select(data, GID_0, country_name, population, area_km2, satellite)

satellite_areas = data[(data$satellite == 1),]
satellite_areas = satellite_areas %>%
  group_by(country_name, satellite) %>%
  summarise(
...
plot1 =
  ...
  geom_text(aes(label = paste(round(population_perc,2),"%")), size=2, vjust=.4,hjust=-.2,
              position = position_dodge(.9), angle=0) +
  theme(legend.position = 'bottom',
        axis.text.x = element_text(angle = 0, hjust=.5, size =8,vjust=1)) +
  labs(title="(A) Population Likely Requiring Satellite by Country.",
       fill=NULL,
       subtitle = "Reported for regions with a population density <5 km^2.",
       x = NULL, y="Population (%)")  +
...
plot2 =
  ...
  labs(title="(B) Geographic Area Likely Requiring Satellite by Country.",
       fill=NULL,
       subtitle = "Reported for regions with a population density <5 km^2.",
