...
for(i in 1:length(my_list)) {
  ...
  ggplot(long, aes(x=strategy_short, y=baseline, fill=strategy_power)) + 
    ...
    labs(title="Impact of Shifting Off-Grid Diesel Generators to Renewable Site Power",
         fill=NULL,
         subtitle = paste("Using a ~10 Mbps per user target with interval bars reflecting low and high adoption scenarios for", country_name),
         x=NULL, y='') + 
  ...
  path = file.path(folder, '..', 'reports', 'images', iso3, 'power_strategies.png')
  ggsave(path, units="in", width=8, height=6, dpi=300)
  while (!is.null(dev.list()))  dev.off()
